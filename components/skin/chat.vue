...
<script setup lang="ts">
	...
	const chat = defineAsyncComponent({
		loader: () => {
			...
			if (isDesktop) {
				return import(`@/components/desktop/model/${model}/chat.vue`)
			}
			return import(`@/components/mobile/model/${model}/chat.vue`)
		},
		delay: 100,
		timeout: 10000,
		suspensible: false,
		onError: (err, retry, error, attempts) => {
			...
			HallLog.error('lazy component: chat is load failed!')
		},
	})
</script>
