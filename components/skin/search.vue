<template>
	<Component :is="homeSearch" :info="info" />
</template>
<script setup lang="ts">
	...
	const homeSearch = defineAsyncComponent({
		loader: () => {
			...
			if (isDesktop) {
				return import(`@/components/desktop/model/${model}/home/search.vue`)
			}
			return import(`@/components/mobile/model/${model}/home/search.vue`)
		},
		delay: 100,
		timeout: 10000,
		suspensible: false,
		onError: (err, retry, error, attempts) => {
			...
			HallLog.error('lazy component: search is load failed!')
		},
	})
</script>
