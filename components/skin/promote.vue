<template>
	<Component :is="homePromote" :info="info" />
</template>
<script setup lang="ts">
	...
	const homePromote = defineAsyncComponent({
		loader: () => {
			//识别路径是否有文件夹，单纯的字符串文件路径不会识别编译到文件路径，会被识别为文件名
			if (isDesktop) {
				return import(`@/components/desktop/model/${model}/home/promote.vue`)
			}
			return import(`@/components/mobile/model/${model}/home/promote.vue`)
		},
		delay: 100,
		timeout: 10000,
		suspensible: false,
		onError: (err, retry, error, attempts) => {
			...
			HallLog.error('lazy component: promote is load failed!')
		},
	})
</script>
