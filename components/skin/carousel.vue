<template>
	<Component :is="homeCarsoule" :info="info" />
</template>
<script setup lang="ts">
	...
	const homeCarsoule = defineAsyncComponent({
		loader: () => {
			//识别路径是否有文件夹，单纯的字符串文件路径不会识别编译到文件路径，会被识别为文件名
			if (isDesktop) {
				return import(`@/components/desktop/model/${model}/home/carousel.vue`)
			}
			return import(`@/components/mobile/model/${model}/home/carousel.vue`)
		},
		...
		onError: (err, retry, error, attempts) => {
			...
			HallLog.error('lazy component: carousel is load failed!')
		},
	})
</script>
