...
<script setup lang="ts">
	...
	const resetComponent = defineAsyncComponent({
		loader: () => {
			//识别路径是否有文件夹，单纯的字符串文件路径不会识别编译到文件路径，会被识别为文件名
			if (isDesktop) {
				return import(`@/components/desktop/model/${model}/reset.vue`)
			}
			return import(`@/components/mobile/model/${model}/reset.vue`)
		},
		...
		onError: (err, retry, error, attempts) => {
			...
			HallLog.error('lazy component: reset is load failed!')
		},
	})
	const resetStore = useResetStore()
	watchEffect(resetStore.changeVerifyText)
	onMounted(resetStore.mounted)
	onUnmounted(resetStore.unmounted)
</script>
