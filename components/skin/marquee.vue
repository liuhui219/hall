<template>
	<Component :is="homeMarquee" :info="info" />
</template>
<script setup lang="ts">
	...
	const homeMarquee = defineAsyncComponent({
		loader: () => {
			//识别路径是否有文件夹，单纯的字符串文件路径不会识别编译到文件路径，会被识别为文件名
			if (isDesktop) {
				return import(`@/components/desktop/model/${model}/home/marquee.vue`)
			}
			return import(`@/components/mobile/model/${model}/home/marquee.vue`)
		},
		...
		onError: (err, retry, error, attempts) => {
			...
			HallLog.error('lazy component: marquee is load failed!')
		},
	})
</script>
