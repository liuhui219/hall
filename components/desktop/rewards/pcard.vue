<template>
	<div class="rewards-card">
		...
		>
			<p class="font-bold text-[16px] max-w-[270px]">{{ $pt(data.cfg.item_lang)?.item_name }}</p>
		...
		<div class="relative z-0 p-[16px] rounded-b-[6px] rewards-card-content-wrap flex flex-col justify-between h-[155px]">
			<p class="text-[12px] font-medium text-lowlight overflow-wrap">{{ $pt(data.cfg.item_lang)?.task_desc }}</p>

			<div class="w-full">
				<button
					v-if="data.used_status"
					class="sys-btn btn-m btn-highlight w-full flex justify-center items-center gap-[8px]"
				...
				<p v-else class="mt-[12px] text-center text-[12px] text-lowlight flex justify-center">
					<template v-if="!!rewardsStore.remainPcardTime(data)">
						{{ $t('REW0004') }}:
						<span class="inline-block min-w-[112px] text-left ml-[4px] color-text-white whitespace-nowrap">{{
