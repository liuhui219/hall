<template>
	<div class="finance-view h-full w-full text-center flex flex-col justify-between">
		<div class="px-4 flex-1 overflow-auto">
			<p class="mt-[16px] text-sm text-white w-full text-center">{{ $t('D0026', { name: 'SPEI' }) }}</p>
			<section class="text-left flex flex-col gap-[16px]">
				<h3 class="withdraw-title-pc">{{ $t('W0030') }}</h3>
		...
		<div class="px-4 pt-4 text-left">
			...
			<button class="sys-btn btn-full-width btn-highlight" @click="withdrawStore.clickWithdraw(PutGroupEnum.spei)">
				{{ $t('U0002') }}
			</button>
		</div>
	</div>
