<template>
	<DesktopPopupHeader
		:invisible="!useRoute().hash?.includes('userInfo') && depositStore.inLobbyPage"
		:back="depositStore.back"
		:title="!depositStore.inResult ? $t('H0005') : ''"
	/>
	<div class="modal__content justify-between flex-1 overflow-hidden flex flex-col">
		<div v-if="depositStore.inLobbyPage" class="flex-1 flex flex-col overflow-hidden relative">
				...
				<section class="mb-8 text-left">
					...
					<ul class="mx-auto">
						...
						>
							...
							<div class="flex items-center">
								<div v-if="item.pay_num_max || item.pay_num_min" class="color-text-white mr-[2px] text-[14px]">
		...
		<div v-else-if="!depositStore.inResult" class="h-full">
		...
		<div
			v-if="depositStore.inLobbyPage && userStore.paytimeTip"
			class="py-[32px] w-full px-[2rem] absolute z-[1] bottom-0 left-0 text-[12px] shrink-0 dark-bg-03"
...
<script setup lang="ts">
	...
	const speiComponent = resolveComponent('desktop/popup/depositSpei')
	const pixComponent = resolveComponent('desktop/popup/depositPix')
	const virtualComponent = resolveComponent('desktop/popup/depositVirtual')
