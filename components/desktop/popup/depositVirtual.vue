<template>
	<div class="finance-view text-center flex flex-col justify-between">
		<div class="px-4 flex-1 overflow-auto">
				...
				<div class="modal-form flex-1 overflow-auto">
					...
					>
						<button
							...
							:key="amount"
							class="sys-btn btn-m amount-btn"
			...
			<section class="text-left">
				<h3 class="text-[14px] font-bold">{{ $t('D0010') }}</h3>
		...
		<div class="px-4 pt-4 text-left">
			<p
				...
				v-html="
					$t('D0020', {
						point: configStore.renderVHtmlNumber($fp(depositStore.config.usdt_rate)),
			...
			<button class="sys-btn btn-m btn-full-width btn-main" @click="depositStore.doUsdtValid()">
				{{ $t('H0005') }}
			</button>
		</div>
	</div>
