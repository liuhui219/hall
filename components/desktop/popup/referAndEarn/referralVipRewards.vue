<template>
	<div class="h-full overflow-hidden flex flex-col">
		<section class="modal-header shrink-0">
			...
			<h3 class="modal__title section-title text-center mb-8">{{ $t('DL0072') }}</h3>
		</section>
		<div
			v-if="$pt(referAndEarnStore.activetyRules.activity38)"
			class="dl-refer-describe !mb-[20px]"
			v-html="$pt(referAndEarnStore.activetyRules.activity38)"
		/>
		<div class="dl-th">
			<div>{{ $t('DL0074') }}</div>
			<div>{{ $t('U0005') }}</div>
				...
				<!-- <div v-for="(item, index) in configStore.inviteUsersCfg.data.awards" :key="index" class="dl-rows">
					<div class="dl-tr">
						<div>
							{{
								$t('DL0062', {
									min: index == 0 ? '1' : configStore.inviteUsersCfg.data.awards[index - 1].max_count + 1,
									max: item.max_count,
								})
							}}
						</div>
...
<style lang="scss" scoped>
	.dl-referral-box {
		@apply px-[16px] h-[666px] overflow-y-auto rounded-b-md;
	...
	.dl-th {
		@apply px-[15px] py-[20px] rounded-t-md;
		background: $block-bg-2;
	}
	.dl-th,
	.dl-tr {
