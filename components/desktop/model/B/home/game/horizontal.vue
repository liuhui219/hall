<template>
	<div v-if="info.list && info.list.length" class="flex flex-col">
		<section class="horizontal-select">
			...
			<DesktopModelBHomeSearch
				v-if="!configStore.pageConfig?.index?.search?.hide"
				class="ml-[24px] w-[25%] max-w-[320px]"
				:info="configStore.pageConfig?.index?.search"
			/>
		</section>
		<section class="home-rank w-full pt-[30px] gap-y-[40px] flex flex-col overflow-hidden">
			<section v-for="(item, index) in renderList" :key="`gc_${active}_${index}`">
