<template>
	...
	<div class="desktop-box" :class="{ active: chatStore.chatHide }">
		...
		<div v-if="chatStore.showMb" class="absolute left-0 w-[100%] h-[100%] z-[999] chatMb" @click="chatStore.closeMb()" />
		<section class="chat-header-wrap relative">
			<button class="back-btn" @click="chatStore.setMobileNavShow()"><BaseIcon name="forward" /></button>
					...
					<div class="flex flex-1 repeat-n">
						<div class="flex-1">
							<div v-if="!item.isRepeat" class="user-info-id">
								...
								<div class="text-[12px]">{{ Helper.formatDate(item.itime, 10) }} {{ Helper.moldHeadImg(item.avatar) }}</div>
							</div>
			...
			<div :class="[chatStore.showSendButton ? 'chat-content show-send' : 'chat-content hide-send']">
				<div class="editor-box">
					<textarea
						...
						:style="{ overflow: chatStore.editScroll ? 'scroll' : 'visible' }"
						spellcheck="false"
						:placeholder="$t('CH008')"
...
<style scoped lang="scss">
	...
	.chat-header-wrap {
		...
		.back-btn {
			@apply bottom-[21px] items-center;
		}
		.back-btn2 {
			@apply shrink-0 bottom-0 pl-[6px] items-center;
		}
		.chat-header-right {
			.chat-help-btn {
				@apply relative;
	...
	.chat-bottom {
		...
		.chat-tab-box {
			...
			.emoji-picker {
				@apply h-[225px] w-[100%] pt-[8px];
				background: $chat-bg-3;
			}
			.chat-tab-span {
				@apply absolute bottom-[-6px] right-[55px] w-0 h-0;
				border-left: 8px solid transparent;
				border-right: 8px solid transparent;
				border-top: 10px solid $chat-bg-3;
			}
			.gif-picker {
