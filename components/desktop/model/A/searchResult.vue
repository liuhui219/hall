<template>
	...
	>
		<div class="wrap px-[30px] flex flex-col min-h-[360px] max-h-[80%] overflow-hidden">
			<div
				class="w-full shrink-0 pb-[40px] items-center bg-bg search-result-content overflow-hidden flex flex-col gap-[20px]"
				style="flex: 1"
				@click.stop="() => {}"
			>
				<div class="px-[27px] flex shrink-0 items-center w-full mt-[2px] py-[25PX] overflow-hidden gap-[20px]">
					...
					<button class="sys-btn shrink-0 min-w-[140px] bg-block-bg color-text-white !h-[60px]" @click="homeStore.closeSearch()">
						{{ $t('T0004') }}
					</button>
				</div>
				...
				<template v-else>
					...
					<template v-else>
						...
						<CompGameHorizontal class="w-full shrink-0 flex-1 px-[27px]" :info="{ ...info, title: 'NH0002' }" :lang="1" :num="8" />
					</template>
				</template>
			</div>
		</div>
...
<script setup lang="ts">
	...
	const styles = computed(() => {
		const left = navigationStore.mobileNavShow ? '320px' : '64px'
		return {
			left: left,
			width: `calc(100% - ${left})`,
		}
