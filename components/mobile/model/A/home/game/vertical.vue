			...
			<div class="flex flex-1 game-list-scroll gap-[8px]">
				...
				<div v-for="(item, index) in configStore.gameGclassList" :key="`ranking-type-${index}`" class="nav-item">
					<button
						...
						class="sys-btn btn-m flex items-center justify-center shrink-0 !text-[14px] home-game-btn"
				...
				<template v-if="!item.provider">
					...
					<div class="game-list flex flex-wrap justify-between home-game">
						<div v-for="count in getCurrentItemCount(index, item)" :key="`${index}-${count}`" class="game-item">
							<BaseGameComponent :provider="true" :game="configStore.getGameByGameId(item.list[count - 1].game_id)" />
						</div>
						<template v-if="getCurrentItemCount(index, item) % 3 != 0">
							<div
								v-for="ids in (3 - (getCurrentItemCount(index, item) % 3)) % 3"
								:key="ids"
								class="game-item opacity-0 pointer-none invisible h-[0]"
							/>
						</template>
					</div>
