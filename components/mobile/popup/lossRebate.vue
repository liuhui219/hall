<template>
	<SkinHeader :is-home="false" :title="$t('V0105')" />
		...
		<div class="px-4 h-full flex flex-col overflow-hidden">
			<div class="flex-1 overflow-auto flex flex-col gap-[20px]">
				<div v-if="userStore.isLogin">
					...
					<div class="vip-rebate-detail flex flex-col gap-[10px] py-[10px]">
						<div>
							<div class="text-lowlight text-[12px] py-[2px] vip-rebate-detail-event px-[10px]">
								{{ $t('V0098') }}{{ vipStore.lossRebateEventFormat }}
							</div>
							<div class="px-[10px] flex items-center">
								<BaseImg
									class="w-[80px] h-[80px] mr-[15px] shrink-0"
									:src="getResConfigImage('loss_rebate', ImageKeyEnum.bonusCenter)"
								/>
								<div class="flex-1 color-text-white">
									<p class="text-[16px] leading-[14.4px] mb-[4px] font-[600]">{{ $t('V0101') }}</p>
									<p
						...
						<div
							class="text-lowlight text-center text-[12px] px-[10px]"
				...
				<div
					...
					:style="{ backgroundImage: `url(${getResConfigImage('loss_rebate_banner', ImageKeyEnum.vip)})` }"
				>
					...
					<div class="font-[400] absolute bottom-[0] w-full text-center text-[12px] py-[3px] color-text-white bg-[rgba(0,0,0,0.2)]">
						{{ $t('V0098') }} {{ vipStore.lossRebateEventFormat }}
					</div>
				</div>
				<div>
					...
					<div class="loss-rebate-rule-container">
						...
						>
							...
							<p class="loss-rebate-rule-cash"><BaseNumber :is-converted="true" :value="item.reward?.flow_pro / 100 || 0" />%</p>
						</div>
					</div>
				</div>
				<div>
	...
	<MobilePopupVipLossRebateReward />
</template>
...
<style lang="scss" scoped>
	.loss-rebate-banner {
		...
		padding-bottom: calc(100% * 170px / 343px);
	...
	.vip-l-detail {
		...
		width: 100%;
		font-size: 12px;
		height: min(calc(100vw * 175px / 375px), 300px);
		background-size: 100% 100%;
		padding: 0 24px 11px;
		position: relative;
		border-radius: 6px;
		...
		.vip-l-detail-container {
			height: calc(min(25.34vw, 200px) + 40px);
			overflow: hidden;
			display: flex;
			justify-content: center;
			flex-direction: column;
		...
		.vip-l-detail-name {
			width: calc(100% - min(25.34vw, 200px) + 20px);
			color: $color-white;
			font-size: 18px;
			font-weight: bold;
			line-height: 20px;
