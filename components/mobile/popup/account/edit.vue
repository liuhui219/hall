	...
	<div class="finance-view finance-view-mobile text-center h-full flex flex-col">
		<section class="px-4 pt-4 pb-8 flex flex-col justify-between h-full">
			...
			<div v-else-if="accountStore.isEditPassword" class="modal-form flex-1">
				<h1>{{ $t('ST0008') }}</h1>
				<h2>{{ $t('ST0009') }}</h2>
				<BaseInput v-model:value="accountStore.editCurrentPassword" :index="1" size="m" :placeholder="$t('U0013')" type="password" />
			...
			<!-- 生日 -->
			<div v-else-if="accountStore.currentType == accountStore.editTypes.birth" class="modal-form flex-1">
				<h1>{{ $t('ST0003') }}</h1>
				<h2>{{ $t('ST0020') }}</h2>
				<BaseDate
		...
		<div class="fixed bottom-[32px] left-0 w-[100%] px-[15px]">
			<button class="sys-btn btn-m w-full btn-highlight" @click="accountStore.saveEdit()">{{ $t('ST0007') }}</button>
		</div>
	</div>
</template>
