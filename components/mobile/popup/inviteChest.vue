<template>
	...
	<div class="modal__content flex flex-col flex-1 overflow-auto min-h-[100%]">
		<div class="relative">
			...
			<div
				class="absolute top-[31.9%] font-bold left-0 right-0 m-auto text-center color-text-white text-[12px] flex items-center justify-center"
				style="color: #2d1183"
			>
				<template v-if="configStore.inviteChestCfg?.data?.reset_period != 4">
					<div class="flex-1 text-right">REDEFINIR EM:&nbsp;</div>
			...
			<div class="absolute flex items-center justify-center w-[76%] left-0 right-0 top-[34.2%] m-auto">
				<div class="color-text-white flex-1 shrink-0 text-center flex flex-col h-[13vw]">
					<div class="scale-[0.83] flex-1 flex items-center justify-center text-[12px] leading-[12px]">{{ $t('IC0002') }}</div>
					<span style="color: #fafb48" class="text-[18px] leading-[15px]">
						<BaseNumber :value="inviteChestStore.data.invite_people_num" />
					</span>
				</div>
				<div class="color-text-white flex-1 shrink-0 text-center flex flex-col h-[13vw]">
					<div class="scale-[0.83] flex-1 flex items-center justify-center text-[12px] leading-[12px]">{{ $t('IC0003') }}</div>
					<span style="color: #fafb48" class="text-[18px] leading-[15px]">
						<BaseNumber :value="inviteChestStore.data.valid_people_num" />
					</span>
				</div>
			</div>
			<div
			...
			<BaseImg :src="getResConfigImage('font_gift', ImageKeyEnum.inviteChest)" alt="" class="absolute top-[52.5%] h-[8vw] left-[6%]" />
			<div class="absolute z-[2] flex flex-wrap justify-between w-full px-[8%] top-[60%] left-0 gap-y-4">
				<div v-for="item in inviteChestStore.data.list" :key="item" class="invite-chest-gift-container">
					<div class="invite-chest-gift-item" :style="{ backgroundImage: `url(${getResConfigImage('back_1', ImageKeyEnum.inviteChest)})` }">
						...
						<p class="invite-chest-gift"><BasePoint :value="item.reward.point" :precision="0" /></p>
					</div>
					<div
			...
			<BaseImg :src="getResConfigImage('font_rules', ImageKeyEnum.inviteChest)" alt="" class="absolute h-[8vw] top-[94.5%] left-[7.5%]" />
		</div>
		<div class="relative">
		...
		<div class="relative -top-[1px] z-[1]">
