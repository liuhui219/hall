<template>
	...
	>
		<div
			class="input-wrap"
			:class="{ 'big-radius': isBigRadius, 'border-none': !hasBorder, 'no-back': noBack, 'input-back': inputBack, 'border-error': error }"
		>
			<slot name="prepend" />
			<input
				ref="inputRef"
		...
		<div v-if="error" class="error-msg text-xs input-error-msg" v-html="errorMessage" />
		<div v-else-if="forceRemind || showRemind" class="text-left text-xs input-error-msg" :class="{ 'text-lowlight': !forceRemind }" v-html="errorMessage" />
	</div>
</template>
